...
This script will save a single csv file in the folder (it will create one if it does not exist)
    ./HNSCC_FUNCTIONAL_ASSAY_MAPPED_DOSE_RESPONSE_DATA
...
if not os.path.exists('./HNSCC_FUNCTIONAL_ASSAY_MAPPED_DOSE_RESPONSE_DATA'):
    os.makedirs('./HNSCC_FUNCTIONAL_ASSAY_MAPPED_DOSE_RESPONSE_DATA')

data.to_csv('./HNSCC_FUNCTIONAL_ASSAY_MAPPED_DOSE_RESPONSE_DATA/HNSCC_funcData_%s.csv' %datetime.today().strftime('%m_%d_%Y'))
