...
assay.dat <- func.dat %>% QC_filter(.) %>% select(lab_id, inhibitor, auc) %>% unique() 

assay.dat <- assay.dat %>% group_by(lab_id, inhibitor) %>% summarize(auc=mean(auc)) %>% ungroup() %>% data.frame()

print(nrow(assay.dat))
print(nrow(assay.dat2))

#print(rownames(assay.dat))
...
dat <- assay.dat %>% filter(lab_id == 10004) %>% arrange(auc.percentile) %>% head(25)  
inhib_order <- dat$inhibitor

plt <-  dat %>% ggplot(aes(x=factor(inhibitor, level=inhib_order), y=auc.percentile)) + geom_col(alpha=0.3) + theme(axis.text.x=element_text(angle=45, hjust=1)) + ggtitle('Functional Data Pipeline QC Filtering') + geom_hline(yintercept=0.2, color='red')
